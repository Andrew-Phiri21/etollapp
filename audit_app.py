...
# --- 2. CORE AUDIT LOGIC (PRECISION ENGINE) ---
...
def check_irregular_plaza(row):
    """
    Evaluates irregular charges based on standard toll rates and plaza-specific fee rules.
    """
    if str(row.get('Reversal_Status', '')).strip().capitalize() == 'Reversed': 
        return "No"
    try:
        amt = abs(float(row.get('_amt_num', 0)))
        plaza = str(row.get('Plaza', '')).upper()
        
        # Standard fees valid across standard stations
        standard_fees = {0, 2, 5, 10, 15, 20, 40, 50, 200, 300, 1000, 3000}
        
        # Fees specific to Michael Chilufya Sata (MCS)
        mcs_fees = {400, 600, 800}
        is_mcs = any(keyword in plaza for keyword in ["MICHAEL", "SATA", "MCS"])
        
        if amt in standard_fees: 
            return "No"
        if is_mcs and amt in mcs_fees: 
            return "No"
        return "Yes"
    except Exception:
        return "No"

def run_analysis(raw_df: pd.DataFrame) -> pd.DataFrame:
    df = raw_df.copy()
    df.columns = df.columns.str.strip()
    
    # Standardize expected columns defensively
    df['DateTime'] = pd.to_datetime(df['Date'], errors='coerce') if 'Date' in df.columns else pd.NaT
    df['reg_clean'] = df['Vehicle Reg.'].apply(clean_vehicle_reg) if 'Vehicle Reg.' in df.columns else ""
    df['Duplicate'] = 'No'
    ...
    amt_col = 'Amount Collected(ZMW)' if 'Amount Collected(ZMW)' in df.columns else 'Amount'
    df['_amt_num'] = pd.to_numeric(df[amt_col], errors='coerce').fillna(0) if amt_col in df.columns else 0.0
    ...
    if 'Card Number' in df.columns and 'DateTime' in df.columns:
        ...
        for i in range(len(active_indices) - 1):
            ...
            if (df.at[curr_idx, 'reg_clean'] == df.at[nxt_idx, 'reg_clean'] and 
                ...
                pd.notna(df.at[curr_idx, 'DateTime']) and pd.notna(df.at[nxt_idx, 'DateTime']) and
    ...
    if not valid_txs.empty:
        modes = valid_txs.groupby('reg_clean')['_amt_num'].apply(
            lambda x: abs(x).mode().iloc[0] if not x.mode().empty else 0
        )
    ...
    def check_exempt(row):
        is_exempt_col = str(row.get('Is Exempt', '')).strip().lower() == 'yes'
        is_free = is_exempt_col or (abs(float(row.get('_amt_num', 0))) == 0)
        if is_free:
            plate = str(row.get('reg_clean', '')).upper()
    ...
    # E. PLAZA-AWARE IRREGULAR CHARGE
...
if uploaded_file:
    if 'main_df' not in st.session_state or st.session_state.get('current_file') != uploaded_file.name:
    ...
    plazas = [p for p in df['Plaza'].unique() if pd.notna(p)] if 'Plaza' in df.columns else []
    plaza_text = ", ".join(map(str, plazas)) if plazas else "Station Not Specified"
    ...
    working_df = df.copy()
    if search_query:
        working_df = working_df[working_df['reg_clean'].astype(str).str.contains(search_query, na=False)]
    ...
    # 2. METRICS
    gross = df['_amt_num'].sum()
    leakage_df = df[df['Audit_Reason'] != ""]
    ...
    display_cols = [c for c in working_df.columns if c not in ['reg_clean', '_amt_num', 'Exempt_Abuse', 'Hour']]
    ...
    receipt_col = 'Receipt No' if 'Receipt No' in active_only.columns else df.columns[0]
    class_stats = active_only.groupby('Category').agg(
        Traffic_Count=(receipt_col, 'count'), 
        Total_Revenue=('_amt_num', 'sum')
    ...
    with c2:
        ...
        if 'Cashier' in df.columns:
            cash_s = df.groupby('Cashier').agg(
                Total=(receipt_col, 'count'), 
                Flags=('Audit_Reason', lambda x: (x != "").sum())
            ).reset_index()
            ...
            st.info("Cashier field not available in uploaded dataset.")

    # --- HOURLY TRAFFIC & LEAKAGE HEATMAP ---
    st.markdown("---")
    ...
    if 'DateTime' in df.columns and df['DateTime'].notna().any():
        ...
        hourly_stats = df.groupby('Hour').agg(
            Total_Traffic=(receipt_col, 'count'),
        ...
        ax_h.set_ylim(0, max(hourly_stats['Total_Traffic'].max() * 1.1, 10))
        ...
        ax2.set_ylim(0, max(hourly_stats['Flagged_Traffic'].max() * 1.5, 5))
        ...
        st.info("DateTime data required for hourly heatmap rendering.")
